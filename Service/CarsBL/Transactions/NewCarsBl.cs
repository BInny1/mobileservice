...
namespace CarsBL.Transactions
{
    public class NewCarsBl
    {
        ...
        public DataSet SaveMobileRegistrationRequest(string Name,string Phone,string Email)
        {
            try
            {
                DataSet dsCars = new DataSet();
                string spNameString = string.Empty;
                Database dbDatabase = DatabaseFactory.CreateDatabase(Global.INSTANCE_NAME);
                spNameString = "[USP_SaveMobileRegistrationRequest]";
                DbCommand dbCommand = null;
                dbCommand = dbDatabase.GetStoredProcCommand(spNameString);

                dbDatabase.AddInParameter(dbCommand, "@RegRequestName", System.Data.DbType.String, Name);
                dbDatabase.AddInParameter(dbCommand, "@RegReqPhoneNumber", System.Data.DbType.String, Phone);
                dbDatabase.AddInParameter(dbCommand, "@RegReqEmail", System.Data.DbType.String, Email);
                dsCars = dbDatabase.ExecuteDataSet(dbCommand);
                return dsCars;
            }
            catch (Exception ex)
